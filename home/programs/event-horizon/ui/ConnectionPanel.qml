...
Item {
 ...
 property var state:(wifi?api.radios.wifi:api.radios.bluetooth)||({})
 property var rows:(wifi?state.networks:state.devices)||[]
 ...
 Row{y:59;spacing:8
  UiButton{id:refresh;width:130;text:p.state.busy||p.state.scanning?'Поиск…':'Обновить';enabled:!!p.state.adapter&&!p.state.busy;onClicked:p.act(p.wifi?'wifi_scan':'bt_scan')}
  UiButton{width:135;text:p.state.enabled?'Включено':'Выключено';selected:p.state.enabled===true;enabled:!!p.state.adapter&&!p.state.busy;onClicked:p.act(p.wifi?'wifi_power':'bt_power',{enabled:!p.state.enabled})}
 }
 UiText{y:110;width:420;text:!p.state.adapter?'Адаптер не найден':!p.state.enabled?'Радиомодуль выключен':p.state.connecting?'Подключение…':p.rows.length?'Выбери сеть или устройство':'Пока ничего не найдено';font.pixelSize:12;color:'#a4c9b7'}
 ListView{id:list;y:142;width:430;height:350;clip:true;spacing:9;model:connectionRows;ScrollBar.vertical:ScrollBar{}
  delegate:Rectangle{
   ...
   Column{visible:card.chosen;x:13;y:83;width:392;spacing:8
    ...
    Row{spacing:8
     UiButton{id:connect;width:192;height:36;enabled:!p.state.busy;text:p.state.busy?'Подключение…':card.modelData.connected?'Отключить':card.modelData.paired||p.wifi?'Подключить':'Сопрячь и подключить'
     ...
     UiButton{visible:!p.wifi&&card.modelData.paired;width:174;height:36;text:p.forgetId===card.modelData.id?'Подтвердить удаление':'Забыть';enabled:!p.state.busy;onClicked:{if(p.forgetId===card.modelData.id){p.act('bt_forget',{id:card.modelData.id});p.forgetId=''}else p.forgetId=card.modelData.id}}
    }
   }
  }
 }
 Rectangle{visible:!p.wifi&&!!p.state.request;x:0;y:142;width:420;height:290;radius:18;color:'#ed102c23';border.width:1;border.color:'#93dfb6'
  property var request:p.state.request||({})
  Column{x:18;y:16;width:384;spacing:14
   ...
   Row{spacing:10
    UiButton{width:174;text:'Подтвердить';visible:!(p.state.request?.kind||'').startsWith('Display');onClicked:{p.act('bt_reply',{accept:true,value:pin.text});pin.clear()}}
    UiButton{width:160;text:'Отмена';onClicked:p.act('bt_reply',{accept:false})}
   }
  }
 }
 UiText{y:507;width:420;height:49;text:p.state.error||'';wrapMode:Text.Wrap;maximumLineCount:4;elide:Text.ElideRight;font.pixelSize:12;color:'#f5c49f'}
