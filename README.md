...
По умолчанию включён космический интерфейс: прозрачные виджеты, календарь,
плеер, уведомления, поиск, Wi-Fi/Bluetooth и меню питания. Emerald также
применяется к настроенным приложениям. После первого `./bin/update`
перезайдите в графический сеанс. Персональные vars можно оставить без изменений.

[Управление, ограничения и откат](home/programs/event-horizon/INTEGRATION.md).
Для прежнего интерфейса установите `programs.eventHorizon = false;` в vars.
Следующий раздел описывает этот прежний вариант.
